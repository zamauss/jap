...
class InformeController extends Controller
{
	...
	{
		$model=Informe::model()->findByPk((int)$id);
		if($model===null)
	...
	public function actionAutocompletesearch()
	{
	    ...
	    if (!empty($q))
	    {
			...
			$criteria->limit='10';
	       	$cursor = Informe::model()->findAll($criteria);
	...
	{
		echo CHtml::tag('option',array(),CHtml::encode('Seleccione un Informe'),true);
		if(Yii::app()->request->isAjaxRequest  )
		{
			...
			{
				if( $relacion[0]==Informe::BELONGS_TO && isset($_POST[$relacion[2]]) && !empty($_POST[$relacion[2]]) )
				{
			...
			if($bandera)
				$data=CHtml::listData(Informe::model()->findAll($criteria), 'id', 'nombre');
			else
				$data=array();

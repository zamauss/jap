...
class PresupuestoController extends Controller
{
	...
	public function actionView($id)
	{
		if(!empty($id))
		{
			$presupuesto = Presupuesto::model()->find('institucion_aid = '.$id);
			$id = $presupuesto->id;
		}
	...
	public function actionCreate()
	{
		...
		if(isset($_GET['institucion_aid']))
		{
			$model->institucion_aid = $_GET['institucion_aid'];
		}
		...
		if(isset($_POST['Presupuesto']))
		{
			...
			$model->institucion_aid = $usuarioActual->institucion->id;
			$model->ejercicioFiscal_did = $ejercicioFiscal->id;
			$model->estatus_did = 1;
			
			if($model->save())
				$this->redirect(array('institucion/termino'));
		}
	...
	{
		$model=Presupuesto::model()->findByPk((int)$id);
		if($model===null)
	...
	{
		echo CHtml::tag('option',array(),CHtml::encode('Seleccione un Presupuesto'),true);
		if(Yii::app()->request->isAjaxRequest  )
		{
			...
			{
				if( $relacion[0]==Presupuesto::BELONGS_TO && isset($_POST[$relacion[2]]) && !empty($_POST[$relacion[2]]) )
				{
			...
			if($bandera)
				$data=CHtml::listData(Presupuesto::model()->findAll($criteria), 'id', 'nombre');
			else
				$data=array();
